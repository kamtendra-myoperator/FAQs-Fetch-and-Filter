...
def generate_questions_and_answers(result_string):
    system_content = '''You are a duplicate remover. 
    The goal is to merge repeated asked question into one.
    Detailed Insructions:
    1. If the "Question" is exact same, then consider only one of them and the most frequent "Answer".
    2. If the "Question" is similar, then make a joint question and joint concise "Answer".
...
def main():
    ...
    output_file = 'testdup.json'
    with open(output_file, 'w') as f:
        json.dump(initial_data, f, indent=4)

    # Define the folder containing conversation transcripts
    folders = ['R1_Filered_FAQs_Chunks_of_25']  # Update with your folder paths
